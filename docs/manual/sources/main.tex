...
% HEADER
    ...
    \begin{center}
        ...
        % Middle Logo
        \includegraphics[width=0.5\linewidth]{FIRST_Tech_challenge_logo.pdf}

        % Space between logos
%        \vspace{1.5cm}

        % Bottom Logo
        \includegraphics[width=0.5\linewidth]{first_age_ftc_decode_logo_vertical_pms_fullcolor.pdf}
    ...
    \chapter*{Démarrage rapide}
    \addcontentsline{toc}{chapter}{Démarrage rapide}

    \begin{myinfo}{Objectif de cette page}
        Cette section vise à fournir toutes les informations essentielles pour commencer rapidement à travailler avec le robot, sans avoir à parcourir l'intégralité du manuel. Elle résume la mise en route, les commandes principales, et les points critiques à vérifier avant chaque session.
    \end{myinfo}


    \section*{Pré-requis matériels et logiciels}
    \begin{itemize}
        \item \textbf{Matériel :} Robot complet, batteries chargées, Gamepads, ordinateur avec Android Studio.
        \item \textbf{Logiciel :} Java 21, Android Studio, FTC SDK, TeamCode téléchargé depuis GitLab.
        \item \textbf{Connexion :} USB ou Wi-Fi avec le Control Hub.
    \end{itemize}


    \section*{Mise en route du robot}
    \begin{enumerate}
        \item Vérifiez que toutes les batteries sont chargées (au moins 12.5 V mesurés dans le Driver Station).
        \item Allumez le Control Hub et le robot.
        \item Connectez le robot à l'ordinateur via USB ou Wi-Fi.
        \item Ouvrez Android Studio, chargez le projet et assurez-vous que Gradle a bien téléchargé toutes les dépendances.
        \item Compilez le code en cliquant sur \enquote{Build}.
        \item Publiez le code sur le robot via \enquote{Start}.
    \end{enumerate}


    \section*{Références rapides}
    \begin{itemize}
        \item \textbf{Manuel officiel FIRST} : \url{https://ftc-resources.firstinspires.org/ftc/game/manual}
        \item \textbf{TeamCode GitLab} : \url{https://gitlab.com/ftc-civ/frites/2026}
        \item \textbf{FTC Dashboard documentation} : \url{https://acmerobotics.github.io/ftc-dashboard/}
    \end{itemize}

    \needspace{10\baselineskip}
    \section*{Commandes principales du robot}
    Le robot est contrôlé via un gamepad standard. Voici les commandes principales:
    \begin{figure}[h]
        \centering
        \includegraphics[width=\linewidth]{GAMEPAD-DIAGRAM.pdf}
        \caption{Schéma des commandes du gamepad pour le robot}
        \label{fig:gamepad-diagram}
    \end{figure}

    \begin{myinfo}{Bon à savoir}
        \begin{itemize}
            \item Toujours vérifier que les batteries sont chargées.
            \item Utiliser \textbf{FTC Dashboard} pour suivre capteurs et vitesse des moteurs.
            \item Le robot prévient si un tir n’est pas possible (flywheel non prête).
        \end{itemize}
    \end{myinfo}
    ...
    \begin{figure}[h]
        ...
        \label{fig:robot-launcher}
    \end{figure}


    \section{Diagrammes et CAD}

    Des diagrammes pour le robot sont disponibles. Ceux-ci sont distribués au format .step, un format standard et libre pour les fichiers CAD.

    \begin{mydefinition}{Comment lire un fichier .step?}
        La majorité des logiciels de CAO (Conception Assistée par Ordinateur) supportent le format .step. Parmi les plus populaires, on retrouve \textit{Fusion 360}, \textit{SolidWorks}, \textit{OnShape} et \textit{FreeCAD}. Il suffit généralement d'ouvrir le fichier directement dans le logiciel pour pouvoir le visualiser et l'éditer.

        Pour visualiser rapidement le modèle 3D sans installer de logiciel, il est aussi possible d'utiliser des visionneuses en ligne comme \url{https://www.viewstl.com/} ou \url{https://3dviewer.net/}. Cependant, le modèle 3D est très lourd (500+ Mo), il est donc recommandé d'utiliser un logiciel local pour une meilleure performance.
    \end{mydefinition}

    \begin{figure}[h]
        \centering
        \includegraphics[width=1\linewidth]{ROBOT-FUSION360.png}
        \caption{Robot prévisualisé dans Fusion 360}
        \label{fig:robot-fusion360}
    \end{figure}

    \subsection{Dessins CAD}
    
Les pages suivantes contiennent tous les dessins CAD qui ont été réalisés. 
Les modèles 3D peuvent être retrouvés sur OnShape :
\begin{itemize}
   \item \href{https://cad.onshape.com/documents/f815cde219ee008354f284c0/w/7517589c4a512dbbaf492a13/e/5a60fd5d0087fbc410132409}{Intake static sorter}
   \item \href{https://cad.onshape.com/documents/5271bfd50433855172e5399a/w/a8057959d9df7047f197dccb/e/a65fa67537320527b7927f01}{Limelight 3A camera holder}
\end{itemize}

    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-RMD.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-ICA.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-L3ACHA.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-LCCA.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-PSCA.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/20991-2526-CSM.pdf}

    \includepdf[pages=-,landscape=true]{Drawings/IntakeStaticSorter.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/LLCameraHolder.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/MecanumBase.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/BatteryHolder.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/NumberPlate.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/GlobalRobotModel.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/Drivetrain.pdf}
    \includepdf[pages=-,landscape=true]{Drawings/IntakeBar.pdf}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=1\linewidth]{circuit_image-2.pdf}
    \end{figure}
    ...
    Veuillez suivre les instructions officielles de \registeredcompany{FIRST} pour le câblage et la configuration des composants.
...
git clone git@gitlab.com:ftc-civ/frites/2026.git
    ...
    (par exemple, le SDK FTC, les pilotes matériels) et ne peuvent pas être modifiées.
...
telemetry = dashboard.getTelemetry();
    ...
    Voici une liste non exhaustive des problèmes courants pouvant survenir lors de l'utilisation du robot, ainsi que des solutions recommandées pour les résoudre :

    \subsubsection{\enquote{pinpoint} ou \enquote{imu} ne répond plus, erreurs de communication}
    Peut apparaître à cause d'un bug dans le control hub ou une interférence électromagnétique, le robot refuse donc de répondre à n'importe quelle action. Pour résoudre ce problème :
    \begin{itemize}
        \item Vérifiez les connexions physiques des capteurs.
        \item Redémarrez le robot avec le bouton de marche/arrêt.
    \end{itemize}

    \subsubsection{Lag/ping très élevé pendant que les flywheels sont activés}

    L'utilisation des flywheels demande énormément de puissance, ce qui peut causer des baisses de tension et des interférences électromagnétiques. Pour résoudre ce problème :
    \begin{itemize}
        \item Assurez-vous que les batteries sont bien chargées (au moins 12.5V). Une batterie complètement chargée affiche environ 14V sur le driver station.
        \item Vérifiez les connexions des moteurs et des ESC.
        \item Si le problème persiste, désactivez temporairement les flywheels pour voir si le lag disparaît.
    \end{itemize}

    \subsubsection{Les déplacements en field centric sont inversés/incorrects}

    Cela est le plus souvent dû à une mauvaise position et orientation détectée dans le système de localisation. Pour résoudre ce problème :
    \begin{itemize}
        \item Démarrer le robot en position correcte sur le terrain.
        \item Regarder des Apriltags pour recalibrer la position.
        \item Si le problème persiste, vérifier la calibration de la Limelight, et s'assurer qu'assez de lumière est présente.
    \end{itemize}

    \subsubsection{Le robot ne ramasse pas les artifacts correctement/les tire sans raison}
    Il faut s'assurer que les artifacts sont ramassés en alternant les côtés de l'intake. De plus, il faut allumer l'intake le temps le plus court possible sur le 3ème artifact pour éviter de bloquer le lanceur.

    \subsubsection{Le driver station empêche de lancer des artifacts}

    Si le driver station empêche de tirer, c'est que la flywheel n'est pas à la bonne vitesse. Solutions:
    \begin{itemize}
        \item S'assurer que la flywheel est bien activée avant de tirer.
        \item Vérifier que la batterie est suffisamment chargée pour alimenter la flywheel.
        \item Si le problème persiste, vérifier les connexions des moteurs et des ESC de la flywheel.
    \end{itemize}
\end{document}